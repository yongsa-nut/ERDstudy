---
title: "Additional Data Analysis for the ERD experiment"
...
print(dfSummary(dat[features_to_sum], 
          ...
          valid.col = FALSE,
          na.col = FALSE,
          tmp.img.dir = "/tmp"), method="render", silent = TRUE)
...
The following figures plot when subjects decide to switch for all trails and condition. 
There are three possible outcomes: 1) subjects switch to an alternative product, 2) subjects receive the initial product, and 3) subjects are forced to switch because the initial product doesn't arrive before the runway.
...
A series of histogram showing the responses on subjective ERD.
There are two times that we ask this question: Week 1 and Week 3. 
...
## Probability before the runway.

A series of boxplots summarizing the probability before the runway across 7 trials.
There are two times that we ask this question: Week 1 and Week 3. 
...
Each factor consists of two results without and with adjustment for multiple comparisons respectively. The adjustment is done by partial pooling. 
...
g_decision <- mcmc_intervals(inv_logit_scaled(to_plot_hierarchal(m_cond_deci_T7_h)), prob_outer = 0.95) + ggtitle("Decision at W1 T7") +
  scale_y_discrete(labels = rev(new_labels), limits = rev) + xlab("Probability of wait")
print(g_decision)

test_all_cond_hypo(m_cond_deci_T7_h,TRUE)

## Manual Hypo for inv_logit_scaled
## The order is ACC, PB, PBL, PBR, RND

# inv_logit_est <- inv_logit_scaled(to_plot_hierarchal(m_cond_deci_T7_h))
# print("Inverse Logit Scale")
# for(i in c(1:4)){
#   for(j in c((i+1):5)){
#     print(paste0(new_labels[i],"-",new_labels[j]) )
#     print(paste0("est: ", round(mean(inv_logit_est[,i] - inv_logit_est[,j]),2) ))
#     print(round(quantile(inv_logit_est[,i] - inv_logit_est[,j], prob=c(.25, .975)),2 ))
#   }
# }
...
g_prob <- mcmc_intervals(to_plot_hierarchal(m_cond_prob_T7_h), prob_outer = 0.95) + ggtitle("Prob before the runway at W1 T7") +  scale_y_discrete(labels = rev(new_labels), limits = rev) + xlab("Probability in percentage")
suppressWarnings(print(g_prob))
...
g_erd <- mcmc_intervals(to_plot_hierarchal(m_cond_ERD_T7_h), prob_outer = 0.95) + ggtitle("Subjective ERD at W1 T7") +
...
plot_grid( g_decision, g_prob, g_erd, g_performance, g_work_again, g_trust, ncol = 3, nrow = 2, 
...
The following set of figures shows the boxplot summary for the probability of receiving the product before the runway and the decision across conditions and trials. 
...
Here, we look specifically at trial 7 for week 1 and 3.
...
g[[1]] + ylab("Probability of waiting at week 1") + xlab("subjective ERD")
...
## Fitting results: Predicting T7 W1 Decisions 

The following results show the fitting results for predicting decisions on week 1 of Trial 7. 
...
#### The code for Bonus payment

```{r eval=FALSE}
##   Code Bonus payment (Paid)
###  For each trial 
